    ...
    <div class="col-md-3">
        <div class="bg-teal-400">
            <div class="bg-secondary p-3 shadow rounded-3 text-white text-center">
                <h2 class="text-semibold mb-1 mt-0">{{ number_with_delimiter(count($list->has_subscriber)-$stats->unsubscribe_email) }} / {{ number_with_delimiter(count($list->has_subscriber)) }}</h2>
...
{{--            @dump ($stats)--}}
            <div class="bg-secondary p-3 shadow rounded-3 text-white text-center">
                <h2 class="text-semibold mb-1 mt-0">{{ number_to_percentage($stats->email_send_percent) }}</h2>
                <div class="text-muted">{{ number_with_delimiter($stats->email_send) }} {{ trans('messages.campaign.successfully_delivered') }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="bg-teal-400">
            <div class="bg-secondary p-3 shadow rounded-3 text-white text-center">
                <h2 class="text-semibold mb-1 mt-0">{{ number_to_percentage($stats->email_failed_percent) }}</h2>
                <div class="text-muted">{{ number_with_delimiter($stats->email_failed) }} {{ trans('messages.campaign.failed_delivery_attempt') }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="bg-teal-400">
            <div class="bg-secondary p-3 shadow rounded-3 text-white text-center">
                <h2 class="text-semibold mb-1 mt-0">{{ number_to_percentage($stats->email_not_send_percent) }}</h2>
                <div class="text-muted">{{ number_with_delimiter($stats->email_not_send) }} {{ trans('messages.campaign.pending_delivery') }}</div>
            </div>
        </div>
    </div>
</div>
...
<div class="listing-form"
        ...
        per-page="{{ App\Customer::$itemsPerPage }}"
    >
                    ...
                    <div class="btn-group list_columns me-2">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            @foreach ($list->getFields as $field)
                                @if ($field->tag != "EMAIL")
                                    <li>
                                        <div class="checkbox">
                                            <label>
                                                <input {{ ($field->required ? "checked='checked'" : "") }} type="checkbox" id="{{ $field->tag }}" name="columns_[]" value="{{ $field->uid }}" class="styled">
                                                {{ $field->label }}
                                            </label>
                                        </div>
                                    </li>
                            ...
                            <li>
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="created_at" name="columns_[]" value="created_at" class="styled">
                            ...
                            <li>
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="updated_at" name="columns_[]" value="updated_at" class="styled">
