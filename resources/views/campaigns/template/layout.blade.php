...
@section('title')
    <i class="material-icons-outlined alert-icon mr-2">backup_table</i>
					...
					<div class="subsection pb-3">
						<h2 class="font-weight-semibold mb-0">
							@if (request()->category_uid)
								{{ App\TemplateCategory::findByUid(request()->category_uid)->name }}
...
search
    ...
    <script>
        ...
        var builderSelectPopup = new Popup({
			onclose: function() {
				window.location = '{{ action('CampaignController@template', [
				'uid' => $campaign->uid,
			]) }}';
			}
		});
