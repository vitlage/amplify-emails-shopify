...
@section('page_header')
	<div class="page-title">
		<ul class="breadcrumb breadcrumb-caret position-right">
			...
			<li class="breadcrumb-item"><a href="{{ action("ListController@index") }}">{{ trans('messages.lists') }}</a></li>
		</ul>
		<h1>
...
@section('content')
	<form action="{{ action('ListController@store') }}" method="POST" class="">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <label for="example-text-input" class=" col-form-label">Add Subscriber<span class="text-danger">*</span></label>

                    <div class="form-group">
                        <select class="select select2-selection--multiple"  name="subscriber[]"data-placeholder="select subscriber..." multiple required>
                            <optgroup label="List Shopify Customers ....">
                                @foreach($s_customers as $s_customer)
                                <option value="{{$s_customer->id}}">{{$s_customer->first_name}} {{$s_customer->last_name}}</option>
                                @endforeach
                            </optgroup> <optgroup label="Other Users ....">
                                @foreach($o_customers as $o_customer)
                                <option value="{{$o_customer->id}}">{{$o_customer->first_name}} {{$o_customer->last_name}}</option>
                                @endforeach
                            </optgroup>
		...
		<div class="text-left">
			...
			<a href="{{ action('ListController@index') }}" class="btn btn-link"><i class="icon-cross2"></i> {{ trans('messages.cancel') }}</a>
		</div>
	</form>
@endsection
